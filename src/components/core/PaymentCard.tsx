    ...
    return (
        <StyledView className="w-full bg-white/10 p-4 flex flex-row justify-between items-center rounded-xl my-1">
            <StyledView className="flex flex-col items-center justify-center">
                <StyledText className="text-white text-base font-bold">{moment(date).format('DD')}</StyledText>
                <StyledText className="text-white text-sm font-medium">{moment(date).format('MMM, YY')}</StyledText>
            </StyledView>
            <StyledView className="flex flex-col">
                <StyledText className="text-white text-base font-medium">Coins: {qty}</StyledText>
                <StyledText className="text-white text-xs font-base uppercase">{payment_type}</StyledText>
            </StyledView>
            <StyledText className="text-white text-lg font-bold">${amount}</StyledText>
        </StyledView>
    )
}