...
export const FloatingButton = ({ navigation }: any) => {
    ...
    return (
        <>
            <StyledTouchableOpacity className="h-14 w-14 rounded-full absolute bottom-16 right-4 bg-white z-10 flex items-center justify-center" onPress={() => setShowCreateModel(!showCreateModel)}>
                <Ionicons name={ showCreateModel? 'close' : 'add'} size={22} color={'black'} />
            </StyledTouchableOpacity>

            <StyledView className={`z-10 absolute bottom-36 right-4 flex gap-y-4 ${showCreateModel ? '' : 'hidden'}`}>
